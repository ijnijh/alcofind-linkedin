...
def render_step1(track: str, series_dict: dict, key_prefix: str):
    ...
    if cands_state:
        ...
        for c in cand_list:
            ...
            # 영역·전략을 짧게 압축 (예: "영역2" / "Area3" / "D")
            theme_short = ""
            if c.get("theme_area"):
                ta = c["theme_area"]
                m = re.search(r"(영역\d+|Area\s*\d+)", ta)
                theme_short = (m.group(0) if m else ta[:10]) + " · "
            hook_short = ""
            if c.get("hook_strategy"):
                hs = c["hook_strategy"]
                m = re.search(r"^[A-H]", hs.strip())
                hook_short = (m.group(0) if m else "?") + " · "
            labels.append(
                f"#{c['idx']} {axis_emoji} {theme_short}{hook_short}{c['title']}"
            )
        choice_idx = st.radio(
            "후보 선택",
            range(len(cand_list)),
        ...
        with st.container(border=True):
            ...
            cax1, cax2 = st.columns([1, 1])
            with cax1:
                st.markdown(f"**🎯 목적 축**  \n{chosen['axis']}")
                if chosen.get("theme_area"):
                    st.markdown(f"**📚 주제 영역**  \n{chosen['theme_area']}")
            with cax2:
                if chosen.get("hook_strategy"):
                    st.markdown(f"**🎣 후킹 전략**  \n{chosen['hook_strategy']}")
                st.markdown(f"**👥 시장 핏**  \n{chosen['fit']}")
